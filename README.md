...
| **_31966_plugin** | 工具箱：表情回应、群成员欢迎、手动及定时睡眠模式控制 |
...
主 AI 私聊和 xiaozhi 桥接均默认关闭，Bot 启动或插件重载不会自动开启。
两者使用独立开关，且仅配置的超级用户/授权用户可以操作：

| 功能 | 开启指令 | 关闭指令 | 说明 |
|------|----------|----------|------|
| `as812` 主 AI 私聊 | `开启私聊` | `关闭私聊` | 仅 `MASTER_UIN` 可操作；关闭时普通私聊不会进入主 AI 对话 |
| xiaozhi 私聊桥接 | `开启小智` | `关闭小智` | 仅 xiaozhi 配置中的 `super_user` / `allowed_users` 可操作；关闭时普通私聊不会发送到 xiaozhi-server |

这两个开关都只在当前运行期间有效，重启后需按需重新开启。私聊转述和管理
指令仍按各插件原有规则处理，不依赖主 AI 私聊开关。
